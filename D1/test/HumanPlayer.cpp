...
int HumanPlayer::ante(int minimumAnte, display gameDisplay) {
    ...
    bannerText << "Current Ante: " << minimumAnte << "; a:see ante; l:leave game";
    gameDisplay.bannerBottom(bannerText.str());

    //check if player has enough money
    if (money < minimumAnte) {
        bannerText << "You do not have enough money to ante. Ending game.";
        gameDisplay.bannerBottom(bannerText.str());
        return Player::FOLD;
    }
        ...
        if (money < minimumBet) {
            ...
            case '\r': case 10: case 'b': case 'B': case 'r': case 'R':
                if (currentBid < minimumBet) {
                    currentBid = 0;
                    bannerText.str("");
                    bannerText << "ERROR: Bid was less than minimum; Minimum: " << minimumBet << "; f:fold; c:call/check; a:all in; current bid: " << currentBid << "  ";
                    gameDisplay.bannerBottom(bannerText.str());
