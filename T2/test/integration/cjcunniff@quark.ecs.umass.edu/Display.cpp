 ...
 * Author: John Shield, Eli Siskind
...
void Display::displayPlayerInfo(Player player) {
	...
	Point p = gameLayout.getPlayerInfoArea(pNum-1);
	int x = p.getX();
	int y = p.getY();
	SmallDeck playerDeck = *(player.getPocket());
	ss << "Display player" << pNum;
	bannerBottom(ss.str());

	// set drawing settings for player
	if(gameLayout.isCurrent(pNum)){
		attr = A_NORMAL | COLOR_PAIR(2);
	}

	//draw border
	drawBox(p, 2 + CARDW * 2, CARDH + 3, attr);
	loc.set(x + 1, y + 2);

	//draw cards
	displayCard(loc, playerDeck.getCard(0), A_NORMAL);
	loc.set(x + 7, y + 2);
	displayCard(loc, playerDeck.getCard(1), A_NORMAL);
 ...
 *		Any suit and number that do not match the valid numbers generates a face down
 *		card.
 ...
 *			display settings.
 */
void Display::drawBox(Point p, int sizeX, int sizeY, int printAtt) {
	int x = p.getX();
	int y = p.getY();
	string strDraw;
	int ii;
