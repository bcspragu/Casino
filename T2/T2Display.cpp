	...
	for (yCount = 0; yCount < sizeY; yCount++) {
		...
		if (x <= cols) {
			...
			for (ii = 0; ii < sizeX; ii++) {
				...
				if (yCount == 0) {
					if (ii == 0) {
						strDraw.append(tl); // left
					} else if (ii == sizeX - 1) {
						strDraw.append(tr); // right
					} else {
					...
					if (ii == 0) {
						strDraw.append(bl); // left
					} else if (ii == sizeX - 1) {
						strDraw.append(br); // right
					} else {
					...
					if (ii == 0) {
						strDraw.append(ml); // left
					} else if (ii == sizeX - 1) {
						strDraw.append(mr); // right
					} else {
...
void T2Display::bannerAd(string bannerText) {
// change to the banner draw settings
	attron(COLOR_PAIR(8) | A_REVERSE);
// checks if the banner string size is smaller than the width of the screen
	if ((unsigned) cols > bannerText.size()) {
